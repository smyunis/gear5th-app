...
func (s HS256HMACValidationService) GetMessage(hashed string) (string, error) {
	messageHash := strings.Split(hashed, s.separator) //[message,hash]

	if len(messageHash) != 2 {
		return "", errors.New("not a valid has generated by infrastructure/HS256HMACValidationService")
	}
	return messageHash[0], nil
}
